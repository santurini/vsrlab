...
class LitBase(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict(
            self.train_metric(
                rearrange(step_out["sr"].detach().clamp(0, 1).contiguous(), 'b t c h w -> (b t) c h w'),
                rearrange(hr.detach().contiguous(), 'b t c h w -> (b t) c h w')
            )
        )
    ...
    def log_images(self, out, stage):
        ...
        lr = F.interpolate(out["lr"][0, -1, :, :, :], size=(c, h, w), mode='trilinear', align_corners=False).detach()

...
class ExampleModel(torch.nn.Module):
    def __init__(self):
        ...
        local_count = sum(
            [torch.numel(param) for name, param in self._moe_layer.named_parameters() if '.experts.' in name])
        shared_count = sum(
            [torch.numel(param) for name, param in self._moe_layer.named_parameters() if '.gate.' in name])
...
tuples = (
dist_world_size, args.dtype, model_dim, hidden_size, batch_size * num_tokens, num_local_experts, top_value, device)
dist_print(
    '[Benchmark] world_size = %s, dtype = %s, model_dim = %s, hidden_size = %s, samples = %s, num_local_experts = %s, topK = %s, device = `%s`' % tuples)
logging.info('Tutel optimized Deepspeed MoE Top-%s = %s' % (top_value, args.use_tutel))

average_time, num_steps = 0, args.num_steps

params_for_all_reduce = [p for p in model.parameters() if
                         not hasattr(p, 'skip_allreduce') and getattr(p, 'requires_grad', False)]
